...
import { COMPONENT_KIND, Frame, Group, Item, carryItemSize, frameOfGroup, frameSizeOf, railExpansionSide, railLayoutWidth, railWidth } from "./tokens";
...
export function constrainModalRails(groups: Group[]): Group[] {
  ...
  const next = groups.map((group) => {
    if (group.items.length === 1 || !group.items.some((it) => it.kind === COMPONENT_KIND.navRail && it.railModal)) return group;
    changed = true;
    return { ...group, items: group.items.map((it) => {
      if (it.kind !== COMPONENT_KIND.navRail || !it.railModal) return it;
...
export const modalRailOf = (group: Group) => group.items.length === 1 && group.items[0].kind === COMPONENT_KIND.navRail && group.items[0].railModal && group.items[0].railExpanded ? group.items[0] : undefined;
...
export function updateRail(groups: Group[], frames: Frame[], widths: Record<string, number>, id: string, patch: Partial<Item>): Group[] {
  const target = groups.find((g) => g.items.some((it) => it.id === id && it.kind === COMPONENT_KIND.navRail));
  ...
  return next.map((g) => {
    if (!owners.has(g.id) || g.locked || g.items.some((it) => it.kind === COMPONENT_KIND.navRail)) return g;
    return {
      ...g,
